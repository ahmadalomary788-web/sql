...
--Q1
--Find orders where total price is greater than the average order price
...
FROM MenuItems
WHERE price = (
    SELECT MAX(price) FROM MenuItems
);
--Q3
-- Find customers who made the latest order
...
--Q4
-- Show all restaurants that have at least one order
...
FROM Customers;
--Q6
--Find menu items that were never ordered (Use NOT IN)
SELECT item_name
FROM MenuItems
WHERE item_id NOT IN (
    SELECT item_id FROM OrderDetails
);

--Q1
--: Show: customer_name, COUNT() of orders Group by customer.
...
--Q2
--Show: customer_name, total number of orders Only for customers who placed more than 3 orders.
SELECT c.customer_name, COUNT(o.order_id) AS total_orders
FROM Customers c
INNER JOIN Orders o ON c.customer_id = o.customer_id
GROUP BY c.customer_id, c.customer_name
...
--Q3
--Show: customer_name, total spent (SUM(price quantity)) Only for customers who spent more than 50 JD total. Use GROUP BY and HAVING.
SELECT c.customer_name, SUM(od.quantity * od.unit_price) AS total_spent
FROM Customers c
INNER JOIN Orders o ON c.customer_id = o.customer_id
INNER JOIN OrderDetails od ON o.order_id = od.order_id
GROUP BY c.customer_id, c.customer_name
HAVING SUM(od.quantity * od.unit_price) > 15.00;
--Q4
--Show: menu_item_name, total quantity sold
...
--Part 2
/* Q1: Why separate tables?
EN: Separating tables stops data duplication, saves storage space, and makes the database faster.

Q2: Problems if names are repeated?
EN: If a restaurant changes its name, you must manually update thousands of old orders, causing errors.
*/
--Part 3
...
/* ==========================================================
   PART 3: DENORMALIZATION (orders_monthly_report)
   ==========================================================

   Q1: Why is this table denormalized?
   EN: Because it merges data from multiple tables into one to speed up reading.

   Q2: Which columns here are redundant?
   EN: The redundant columns are customer_name and restaurant_name.

   Q3: What is the main advantage of this table for reports?
   EN: Extreme speed in generating reports without needing slow and complex JOINs.

   Q4: What is the main disadvantage and how to keep it updated?
   EN: The flaw is stale data. It is updated using scheduled Batch Jobs or database Triggers.
========================================================== */